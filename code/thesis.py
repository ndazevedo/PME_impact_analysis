...
##### MUNIC VARIABLES 2018
# Caracterização do órgão gestor 					MEDU01
# 					
# Sexo					MEDU03
# Idade 					MEDU04
# Cor/raça					MEDU05
# 	Este quesito foi respondido pelo próprio titular				MEDU051
# Escolaridade					MEDU06
# 	Formação de nível superior				MEDU07
# 		Especifique			MEDU07111
# 					
# Concluiu, nos últimos 5 anos, curso de capacitação em gestão educacional					MEDU08
# 	Programa do curso oferecido				MEDU081


# Plano Municipal de Educação - existência					MEDU14
# 	Lei do atual PEE				MEDU141A
# 	Ano do atual PEE				MEDU141B

# 3.7 Diretores das escolas da rede municipal de ensino					
# Nomeação ocorre por:					
# 	Concurso Público (para carreira de diretor escolar constituída por cargo público específico de diretor escolar)				MEDU491
# 	Livre nomeação (para cargo ou função comissionada)				MEDU492
# 		Indicação			MEDU49211
# 		Eleição			MEDU49212
# 		Processo seletivo			MEDU49213
# 		Outro			MEDU49214
...
## Arranging dataframe and exogenous variables
df_fix = df[df['year'].isin(analysis_years_2015)].copy()
...
######### HETEROGENEITY
### office
math_model_office = sm.OLS(df_fix[df_fix['office']==1]['math_score'], df_fix[df_fix['office']==1][exog_vars])
reading_model_office = sm.OLS(df_fix[df_fix['office']==1]['reading_score'], df_fix[df_fix['office']==1][exog_vars])

math_results_office = math_model_office.fit()
reading_results_office = reading_model_office.fit()

print(math_results_office.summary())
print(reading_results_office.summary())

### states
north = list(range(11, 18))
north_east = list(range(21, 30))
south_east = [31, 32, 33, 35]
south = [41, 42, 43]
midwest = [50, 51, 52, 53]

def assign_region(co_uf):
    if co_uf in north:
        return 'north'
    elif co_uf in north_east:
        return 'north_east'
    elif co_uf in south_east:
        return 'south_east'
    elif co_uf in south:
        return 'south'
    elif co_uf in midwest:
        return 'midwest'
    else:
        return None

regions = ['north', 'north_east', 'south_east', 'south', 'midwest']

# Apply function to create new column 'region'
df_fix['region'] = df_fix['CO_UF'].apply(assign_region)

def het_reg_func(filt_col, value):
    math_model_1 = sm.OLS(df_fix[df_fix[filt_col]==value]['math_score'], df_fix[df_fix[filt_col]==value][exog_vars])
    reading_model_1 = sm.OLS(df_fix[df_fix[filt_col]==value]['reading_score'], df_fix[df_fix[filt_col]==value][exog_vars])
    
    math_results_1 = math_model_1.fit()
    reading_results_1 = reading_model_1.fit()
    
    print(math_results_1.summary())
    print(reading_results_1.summary())


################## 2017
## Arranging dataframe and exogenous variables
df_fix = df[df['year'].isin(analysis_years_2017)].copy()

df_fix = df_fix.drop(columns=['approval_rates']).dropna()
df_fix['const'] = 1
df_fix['post'] = df_fix['year'].apply(lambda x: 1 if x > 2014 else 0)
df_fix['treat_post'] = df_fix['treatment'] * df_fix['post']
df_fix = df_fix.set_index(['CODMUN', 'year'])

# ex_vars = ['const', 'post', 'treatment', 'treat_post']
exog_vars = df.columns[8:14].tolist()
exog_vars.extend(['students' ,'gdp', 'const', 'post', 'treatment', 'treat_post'])

math_model = sm.OLS(df_fix['math_score'], df_fix[exog_vars], entity_effects=True)
math_results = math_model.fit()

print(math_results.summary())

reading_model = sm.OLS(df_fix['reading_score'], df_fix[exog_vars], entity_effects=True)
reading_results = reading_model.fit()

print(reading_results.summary())

# print(math_results_cluster.summary())
# print(reading_results_cluster.summary())
...
################## 2019
## Arranging dataframe and exogenous variables
df_fix = df[df['year'].isin(analysis_years_2019)].copy()
...
## Arranging dataframe and exogenous variables
df_fix = df_2[df_2['year'].isin(analysis_years_2015)].copy()
...
math_model = sm.OLS(df_fix['math_score'], df_fix[exog_vars])
math_results = math_model.fit()

print(math_results.summary())

reading_model = sm.OLS(df_fix['reading_score'], df_fix[exog_vars])
reading_results = reading_model.fit()
...
################## 2017
## Arranging dataframe and exogenous variables
df_fix = df_2[df_2['year'].isin(analysis_years_2017)].copy()
...
################## 2019
## Arranging dataframe and exogenous variables
df_fix = df_2[df_2['year'].isin(analysis_years_2019)].copy()
